...
// Create returns an explicit error instead of leaving computed attributes
// unknown. This exercises the len(TenantImages) == 0 branch after import.
...
// TestUnitTenantImageCreatePostImportEmptyImages verifies that when GetImage
// returns successfully after import but with an empty TenantImages list,
// Create still writes state (Id is set) but the Computed "status" attribute
// remains unknown, which Terraform's framework treats as a provider bug.
// This test exercises the len(TenantImages) == 0 branch after import and
// expects the framework error about unknown values after apply.
func TestUnitTenantImageCreatePostImportEmptyImages(t *testing.T) {
	testAccPreUnitCheck(t)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccTenantImageCreateTC2ResourceConfig,
				// When GetImage returns empty after import, status stays
				// unknown — the framework rejects this with an error about
				// invalid result object. This exercises the empty branch.
				ExpectError: regexp.MustCompile(`(?i)unknown value|invalid result`),
			},
		},
	})
}
