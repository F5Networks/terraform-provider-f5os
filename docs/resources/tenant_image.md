...
```terraform
# Resource for uploading tenant image
resource "f5os_tenant_image" "test" {
  image_name       = "BIGIP-17.1.0-0.0.16.ALL-F5OS.qcow2.zip.bundle"
  upload_from_path = "/User/xyz/Documents"
}
```
...
- `upload_from_path` (string) The path on the client machine where the image file resides. This attribute is used when the the user wants to upload the image from his/her machine.
